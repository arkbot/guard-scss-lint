...
describe Guard::ScssLint, :silence_output do
  ...
  shared_examples 'processes after running', :processes_after_running do
    context 'when passed' do
      it 'throws nothing' do
        allow_any_instance_of(Guard::ScssLint::Runner).to receive(:run).and_return(true)
        expect { subject }.not_to throw_symbol
      end

      it 'clears failed paths' do
        allow_any_instance_of(Guard::ScssLint::Runner).to receive(:run).and_return(true)
        allow_any_instance_of(Guard::ScssLint::Runner).to receive(:failed_paths).and_return([])
        subject
    ...
    context 'when failed' do
      it 'throws symbol :task_has_failed' do
        allow_any_instance_of(Guard::ScssLint::Runner).to receive(:run).and_return(false)
      ...
      it 'keeps failed paths' do
        ...
        allow_any_instance_of(Guard::ScssLint::Runner).to receive(:run).and_return(false)
        allow_any_instance_of(Guard::ScssLint::Runner)
    ...
    context 'when an exception is raised' do
      it 'prevents itself from firing' do
        allow_any_instance_of(Guard::ScssLint::Runner).to receive(:run).and_raise(RuntimeError)
  ...
  describe '#run_all', :processes_after_running do
    ...
    it 'inspects all files with scss-lint' do
      expect_any_instance_of(Guard::ScssLint::Runner).to receive(:run).with([])
      guard.run_all
    end
  end
  ...
  [:run_on_additions, :run_on_modifications].each do |method|
    describe "##{method}", :processes_after_running do
      ...
      it 'inspects changed files with scss-lint' do
        expect_any_instance_of(Guard::ScssLint::Runner).to receive(:run)
        subject
      end

      it 'passes cleaned paths to scss-lint' do
        expect_any_instance_of(Guard::ScssLint::Runner).to receive(:run) do |_instance, paths|
          expect(paths).to eq([
            File.expand_path('lib/guard/scss-lint.rb'),
      ...
      context 'when cleaned paths are empty' do
        ...
        it 'does nothing' do
          expect_any_instance_of(Guard::ScssLint::Runner).not_to receive(:run)
          subject
        end
      end
